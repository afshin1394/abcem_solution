...
# Reflect the tables in sms schema
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        version_table="sms",  # <--- custom version table
        literal_binds=True,
        compare_type=True,
...
def run_migrations_online() -> None:
    ...
    connectable = engine_from_config(
        ...
        connect_args={"options": "-c search_path=sms"},  # <--- Important
    )
    with connectable.connect() as connection:
        connection.execute(text('CREATE SCHEMA IF NOT EXISTS sms'))
        print(f"Loaded tables in target metadata: {list(target_metadata.tables.keys())}")

        context.configure(
            connection=connection, target_metadata=target_metadata,
            version_table_schema="sms",  # Important!
