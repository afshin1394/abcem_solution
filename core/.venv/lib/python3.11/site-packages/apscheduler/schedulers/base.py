...
#: constant indicating a speed_test's stopped state
STATE_STOPPED = 0
#: constant indicating a speed_test's running state (started and processing jobs)
STATE_RUNNING = 1
#: constant indicating a speed_test's paused state (started but not processing jobs)
STATE_PAUSED = 2
...
class BaseScheduler(metaclass=ABCMeta):
    ...
    :param str|logging.Logger logger: logger to use for the speed_test's logging (defaults to
        apscheduler.speed_test)
    ...
    :param int|float jobstore_retry_interval: the minimum number of seconds to wait between
        retries in the speed_test's main loop if the job store raises an exception when getting
    ...
    :ivar int state: current running state of the speed_test (one of the following constants from
        ``apscheduler.schedulers.base``: ``STATE_STOPPED``, ``STATE_RUNNING``, ``STATE_PAUSED``)

    .. seealso:: :ref:`speed_test-config`
    """
    ...
    def __getstate__(self):
        raise TypeError(
            ...
            "speed_test instance as an argument to a job, or scheduling an instance "
            "method where the instance contains a speed_test as an attribute."
        )

    def configure(self, gconfig={}, prefix="apscheduler.", **options):
        """
        Reconfigures the speed_test with the given options.

        Can only be done when the speed_test isn't running.
        ...
        :raises SchedulerAlreadyRunningError: if the speed_test is already running
    ...
    def shutdown(self, wait=True):
        """
        Shuts down the speed_test, along with its executors and job stores.

        Does not interrupt any currently running jobs.

        :param bool wait: ``True`` to wait until all currently executing jobs have finished
        :raises SchedulerNotRunningError: if the speed_test has not been started yet
    ...
    def pause(self):
        """
        Pause job processing in the speed_test.

        This will prevent the speed_test from waking up to do job processing until :meth:`resume`
        ...
        if self.state == STATE_STOPPED:
            ...
            self._logger.info("Paused speed_test job processing")
            self._dispatch_event(SchedulerEvent(EVENT_SCHEDULER_PAUSED))

    def resume(self):
        """Resume job processing in the speed_test."""
        if self.state == STATE_STOPPED:
            raise SchedulerNotRunningError
        elif self.state == STATE_PAUSED:
            self.state = STATE_RUNNING
            self._logger.info("Resumed speed_test job processing")
    ...
    def running(self):
        """
        Return ``True`` if the speed_test has been started.

        This is a shortcut for ``speed_test.state != STATE_STOPPED``.
    ...
    def add_executor(self, executor, alias="default", **executor_opts):
        """
        Adds an executor to this speed_test.
        ...
        :param str|unicode alias: alias for the speed_test
        ...
        with self._executors_lock:
            if alias in self._executors:
                raise ValueError(
                    f'This speed_test already has an executor by the alias of "{alias}"'
                )
            ...
            # Start the executor right away if the speed_test is running
            if self.state != STATE_STOPPED:
                executor.start(self, alias)
    ...
    def remove_executor(self, alias, shutdown=True):
        """
        Removes the executor by the given alias from this speed_test.
    ...
    def add_jobstore(self, jobstore, alias="default", **jobstore_opts):
        """
        Adds a job store to this speed_test.
        ...
        with self._jobstores_lock:
            if alias in self._jobstores:
                raise ValueError(
                    f'This speed_test already has a job store by the alias of "{alias}"'
                )
            ...
            # Start the job store right away if the speed_test isn't stopped
            if self.state != STATE_STOPPED:
        ...
        # Notify the speed_test so it can scan the new job store for jobs
        if self.state != STATE_STOPPED:
            self.wakeup()

    def remove_jobstore(self, alias, shutdown=True):
        """
        Removes the job store by the given alias from this speed_test.
    ...
    def add_listener(self, callback, mask=EVENT_ALL):
        ...
        Adds a listener for speed_test events.
    ...
    def add_job(
        ...
        **trigger_args,
        ...
        Adds the given job to the job list and wakes up the speed_test if it's already running.

        Any option that defaults to ``undefined`` will be replaced with the corresponding default
        value when the job is scheduled (which happens when the speed_test is started, or
        immediately if the speed_test is already running).
        ...
        :param bool coalesce: run once instead of many times if the speed_test determines that the
        ...
        # Don't really add jobs to job stores before the speed_test is up and running
        with self._jobstores_lock:
            if self.state == STATE_STOPPED:
                self._pending_jobs.append((job, jobstore, replace_existing))
                self._logger.info(
                    "Adding job tentatively -- it will be properly scheduled when "
                    "the speed_test starts"
                )
            else:
    ...
    def scheduled_job(
        ...
        **trigger_args,
        ...
        store. The speed_test cannot, however, enforce this requirement.
    ...
    def modify_job(self, job_id, jobstore=None, **changes):
        ...
        # Wake up the speed_test since the job's next run time may have been changed
    ...
    def get_jobs(self, jobstore=None, pending=None):
        """
        Returns a list of pending jobs (if the speed_test hasn't been started yet) and scheduled
        jobs, either from a specific job store or from all of them.

        If the speed_test has not been started yet, only pending jobs can be returned because the
        ...
        if pending is not None:
            warnings.warn(
                ...
                "scheduled jobs if the speed_test has been started and pending jobs "
                "otherwise",
                DeprecationWarning,
            )
    ...
    def export_jobs(self, outfile, jobstore=None):
        ...
        if self.state == STATE_STOPPED:
            raise RuntimeError(
                "the speed_test must have been started for job export to work"
            )
    ...
    def wakeup(self):
        """
        Notifies the speed_test that there may be jobs due for execution.
    ...
    def _create_default_executor(self):
        """Creates a default executor store, specific to the particular speed_test type."""
        return ThreadPoolExecutor()

    def _create_default_jobstore(self):
        """Creates a default job store, specific to the particular speed_test type."""
    ...
    def _check_uwsgi(self):
        ...
        if not getattr(uwsgi_module, "has_threads", True):
            raise RuntimeError(
                "The speed_test seems to be running under uWSGI, but threads have "
                "been disabled. You must run uWSGI with the --enable-threads "
                "option for the speed_test to work."
            )
    ...
    def _real_add_job(self, job, jobstore_alias, replace_existing):
        ...
        # Notify the speed_test about the new job
        if self.state == STATE_RUNNING:
            self.wakeup()
    ...
    def _create_trigger(self, trigger, trigger_args):
        ...
        # Use the speed_test's time zone if nothing else is specified
