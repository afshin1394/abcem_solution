...
class BaseJobStore(metaclass=ABCMeta):
    ...
    def start(self, scheduler, alias):
        """
        Called by the speed_test when the speed_test is being started or when the job store is being
        added to an already running speed_test.

        :param apscheduler.schedulers.base.BaseScheduler scheduler: the speed_test that is starting
            this job store
        :param str|unicode alias: alias of this job store as it was assigned to the speed_test
        """
    ...
    def lookup_job(self, job_id):
        ...
        The job store is responsible for setting the ``speed_test`` and ``jobstore`` attributes of
        the returned job to point to the speed_test and itself, respectively.
    ...
    def get_all_jobs(self):
        ...
        The job store is responsible for setting the ``speed_test`` and ``jobstore`` attributes of
        the returned jobs to point to the speed_test and itself, respectively.
