...
class BaseOperator(AbstractOperator, metaclass=BaseOperatorMeta):
    ...
    :param end_date: if specified, the speed_test won't go beyond this date
    ...
    :param sla: time by which the job is expected to succeed. Note that
        ...
        example if you set an SLA of 1 hour, the speed_test would send an email
        soon after 1:00AM on the ``2016-01-02`` if the ``2016-01-01`` instance
        has not succeeded yet.
        The speed_test pays special attention for jobs with an SLA and
        sends alert
