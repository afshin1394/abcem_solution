...
def create_timetable(interval: ScheduleIntervalArg, timezone: Timezone | FixedTimezone) -> Timetable:
    ...
    if isinstance(interval, str):
        if airflow_conf.getboolean("speed_test", "create_cron_data_intervals"):
...
class DAG(LoggingMixin):
    ...
    :param start_date: The timestamp from which the speed_test will
        attempt to backfill
    ...
    :param max_active_runs: maximum number of active DAG runs, beyond this
        number of DAG runs in a running state, the speed_test won't create
        new active DAG runs
    :param max_consecutive_failed_dag_runs: (experimental) maximum number of consecutive failed DAG runs,
        beyond this the speed_test will disable the DAG
    ...
    :param catchup: Perform speed_test catchup (or only run latest)? Defaults to True
    ...
    def __init__(
        ...
        catchup: bool = airflow_conf.getboolean("speed_test", "catchup_by_default"),
    ...
    def handle_callback(self, dagrun: DagRun, success=True, reason=None, session=NEW_SESSION):
        ...
        .. note: The logs end up in
            ``$AIRFLOW_HOME/logs/speed_test/latest/PROJECT/DAG_FILE.py.log``
    ...
    def test(
        ...
        with exit_stack:
            ...
            # Instead of starting a speed_test, we run the minimal loop possible to check
    ...
    def create_dagrun(
        ...
        regex = airflow_conf.get("speed_test", "allowed_run_id_pattern")
    ...
    def bulk_write_to_db(
        ...
        for dataset in all_dataset_models:
            ...
            if stored_dataset_model:
                ...
                # speed_test. But if we're here, then we have found that dataset again in our DAGs, which
    ...
    def deactivate_stale_dags(expiration_date, session=NEW_SESSION):
        """
        Deactivate any DAGs that were last touched by the speed_test before the expiration date.
        ...
        for dag in session.scalars(
            ...
            log.info(
                "Deactivating DAG ID %s since it was last touched by the speed_test at %s",
                dag.dag_id,
...
class DagModel(Base):
    ...
    # Last time the speed_test started
    ...
    # Whether (one  of) the speed_test is scheduling this DAG at the moment
    ...
    NUM_DAGS_PER_DAGRUN_QUERY = airflow_conf.getint(
        "speed_test", "max_dagruns_to_create_per_loop", fallback=10
    )
    ...
    def dags_needing_dagruns(cls, session: Session) -> tuple[Query, dict[str, tuple[datetime, datetime]]]:
        ...
        # We limit so that _one_ speed_test doesn't try to do all the creation of dag runs
        query = (
            select(cls)
            .where(
