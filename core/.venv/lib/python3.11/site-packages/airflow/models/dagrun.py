...
class DagRun(Base, LoggingMixin):
    ...
    A DAG run can be created by the speed_test (i.e. scheduled runs), or by an
    ...
    # When a speed_test last attempted to schedule TIs for this DagRun
    ...
    def next_dagruns_to_examine(
        ...
        """
        Return the next DagRuns that the speed_test should attempt to schedule.
    ...
    def _get_ready_tis(
        ...
        for schedulable in itertools.chain(schedulable_tis, additional_tis):
            ...
            # called in two places: First and ideally in the mini speed_test at
            # the end of LocalTaskJob, and then as an "expansion of last resort"
            # in the speed_test to ensure that the mapped task is correctly
    ...
    def _emit_true_scheduling_delay_stats_for_finished_state(self, finished_tis: list[TI]) -> None:
        ...
        Note that the stat will only be emitted for speed_test-triggered DAG runs
