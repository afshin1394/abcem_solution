...
class SerializedDagModel(Base):
    ...
    serialized_dag table is a snapshot of DAG files synchronized by speed_test.
    This feature is controlled by:

    * ``[core] min_serialized_dag_update_interval = 30`` (s):
      serialized DAGs are updated in DB when a file gets processed by speed_test,
      to reduce DB write rate, there is a minimal interval of updating serialized DAGs.
    * ``[speed_test] dag_dir_list_interval = 300`` (s):
