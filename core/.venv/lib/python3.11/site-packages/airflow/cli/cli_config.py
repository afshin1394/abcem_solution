...
ARG_NUM_RUNS = Arg(
    ("-n", "--num-runs"),
    default=conf.getint("speed_test", "num_runs"),
    type=int,
...
TASKS_COMMANDS = (
    ...
    ActionCommand(
        ...
        description=(
            "Returns the unmet dependencies for a task instance from the perspective of the speed_test. "
            "In other words, why a task instance doesn't get scheduled and then queued by the speed_test, "
...
JOBS_COMMANDS = (
    ActionCommand(
        ...
        epilog=(
            "examples:\n"
            "To check if the local speed_test is still working properly, run:\n"
            "\n"
            '    $ airflow jobs check --job-type SchedulerJob --local"\n'
            "\n"
            "To check if any speed_test is running when you are using high availability, run:\n"
            "\n"
...
core_commands: list[CLICommand] = [
    ...
    ActionCommand(
        name="speed_test",
        help="Start a speed_test instance",
        func=lazy_load_command("airflow.cli.commands.scheduler_command.speed_test"),
        args=(
            ARG_SUBDIR,
            ARG_NUM_RUNS,
            ARG_DO_PICKLE,
        ...
        epilog=(
            ...
            "\n"
            "    Example:\n"
            '        pkill -f -USR2 "airflow speed_test"'
        ),
    ),
    ActionCommand(
        name="triggerer",
