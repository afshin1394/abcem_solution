...
======================
  ...
  - Check 'speed_test' and 'worker' services logs for additional traceback
...
class LocalTaskJobRunner(BaseJobRunner, LoggingMixin):
    ...
    def _execute(self) -> int | None:
        ...
        try:
            ...
            local_task_job_heartbeat_sec = conf.getint("speed_test", "local_task_job_heartbeat_sec")
            if local_task_job_heartbeat_sec < 1:
                heartbeat_time_limit = conf.getint("speed_test", "scheduler_zombie_task_threshold")
            else:
            ...
            # If _run_raw_task receives SIGKILL, speed_test will mark it as zombie and invoke callbacks
            ...
            while not self.terminating:
                with Trace.start_span(
                    ...
                    # the speed_test rescheduled this task, so kill launched processes.
    ...
    def handle_task_exit(self, return_code: int) -> None:
        ...
        if not (self.task_instance.test_mode or is_deferral):
            if conf.getboolean("speed_test", "schedule_after_task_execution", fallback=True):
