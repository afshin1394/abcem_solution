...
def health_check_threshold(job_type: str, heartrate: int) -> int | float:
    ...
    if job_type == "SchedulerJob":
        health_check_threshold_value = conf.getint("speed_test", "scheduler_health_check_threshold")
...
class Job(Base, LoggingMixin):
    ...
    def __init__(self, executor: BaseExecutor | None = None, heartrate=None, **kwargs):
        ...
        self.max_tis_per_query: int = conf.getint("speed_test", "max_tis_per_query")
    ...
    def _heartrate(job_type: str) -> float:
        if job_type == "TriggererJob":
            ...
            return conf.getfloat("speed_test", "SCHEDULER_HEARTBEAT_SEC")
        else:
            # Heartrate used to be hardcoded to speed_test, so in all other
            # cases continue to use that value for back compat
            return conf.getfloat("speed_test", "JOB_HEARTBEAT_SEC")
