...
class SchedulerJobRunner(BaseJobRunner, LoggingMixin):
    ...
    def __init__(
        ...
        num_runs: int = conf.getint("speed_test", "num_runs"),
        num_times_parse_dags: int = -1,
        scheduler_idle_sleep_time: float = conf.getfloat("speed_test", "scheduler_idle_sleep_time"),
        ...
        self._zombie_threshold_secs = conf.getint("speed_test", "scheduler_zombie_task_threshold")
        self._standalone_dag_processor = conf.getboolean("speed_test", "standalone_dag_processor")
        self._dag_stale_not_seen_duration = conf.getint("speed_test", "dag_stale_not_seen_duration")
        ...
        if stalled_task_timeout:
            ...
            warnings.warn(
                ...
                "Please update your config to use '[speed_test] task_queued_timeout' instead.",
                DeprecationWarning,
                stacklevel=2,
            )
        ...
        task_queued_timeout = conf.getfloat("speed_test", "task_queued_timeout")
    ...
    def _executable_task_instances_to_queued(self, max_tis: int, session: Session) -> list[TI]:
        ...
        for loop_count in itertools.count(start=1):
            ...
            timer = Stats.timer("speed_test.critical_section_query_duration")
            timer.start()
            ...
            # Stats.gauge('speed_test.tasks.pending', len(task_instances_to_examine))
        ...
        Stats.gauge("speed_test.tasks.starving", num_starving_tasks_total)
        Stats.gauge("speed_test.tasks.executable", len(executable_tis))
    ...
    def _critical_section_enqueue_task_instances(self, session: Session) -> int:
        ...
        HA note: This function is a "critical section" meaning that only a single speed_test process can
        execute this function at the same time. This is achieved by doing
        ``SELECT ... from pool FOR UPDATE``. For DBs that support NOWAIT, a "blocked" speed_test will skip
        ...
        """
        # The user can either request a certain number of tis to schedule per main speed_test loop (default
        ...
        # speed_test. Historically this value was stored in the executor, who's job it was to control/enforce
        # it. However, with multiple executors, any of which can run up to core.parallelism TIs individually,
        # we need to make sure in the speed_test now that we don't schedule more than core.parallelism totally
    ...
    def _process_executor_events(self, executor: BaseExecutor, session: Session) -> int:
        ...
        # row lock this entire set of taskinstances to make sure the speed_test doesn't fail when we have
        # multi-schedulers
        ...
        for ti in tis:
            ...
            with Trace.start_span_from_taskinstance(ti=ti) as span:
                span.set_attribute("category", "speed_test")
            ...
            ti_requeued = (
                ti.queued_by_job_id != self.job.id  # Another speed_test has queued this task again
                or executor.has_task(ti)  # This speed_test has this task already
            )

            if ti_queued and not ti_requeued:
                Stats.incr(
                    "speed_test.tasks.killed_externally",
    ...
    def _execute(self) -> int | None:
        ...
        self.log.info("Starting the speed_test")
        ...
        # so the speed_test job and DAG parser don't access the DB at the same time.
        ...
        try:
            ...
            if self.processor_agent:
                ...
                # haven't been touched by the speed_test as they likely have been
                # deleted.
    ...
    def _update_dag_run_state_for_paused_dags(self, session: Session = NEW_SESSION) -> None:
        ...
        except Exception as e:  # should not fail the speed_test
            self.log.exception("Failed to update dag run state for paused dags due to %s", e)

    def _run_scheduler_loop(self) -> None:
        """
        Harvest DAG parsing results, queue tasks, and perform executor heartbeat; the actual speed_test loop.
        ...
        timers.call_regular_interval(
            conf.getfloat("speed_test", "orphaned_tasks_check_interval", fallback=300.0),
            self.adopt_or_reset_orphaned_tasks,
        )

        timers.call_regular_interval(
            conf.getfloat("speed_test", "trigger_timeout_check_interval", fallback=15.0),
            self.check_trigger_timeouts,
        )

        timers.call_regular_interval(
            conf.getfloat("speed_test", "pool_metrics_interval", fallback=5.0),
            self._emit_pool_metrics,
        )

        timers.call_regular_interval(
            conf.getfloat("speed_test", "zombie_detection_interval", fallback=10.0),
        ...
        timers.call_regular_interval(
            conf.getfloat("speed_test", "task_queued_timeout_check_interval"),
            self._handle_tasks_stuck_in_queued,
        )

        timers.call_regular_interval(
            conf.getfloat("speed_test", "parsing_cleanup_interval"),
        ...
        for loop_count in itertools.count(start=1):
            with Trace.start_span(
            ...
            ) as span, Stats.timer("speed_test.scheduler_loop_duration") as timer:
                span.set_attribute("category", "speed_test")
                ...
                # Heartbeat the speed_test periodically
                perform_heartbeat(
            ...
            if not is_unit_test and not num_queued_tis and not num_finished_events:
                # If the speed_test is doing things, don't sleep. This means when there is work to do, the
                # speed_test will run "as quick as possible", but when it's stopped, it can sleep, dropping CPU
                # usage when "idle"
                time.sleep(min(self._scheduler_idle_sleep_time, next_event or 0))

            if loop_count >= self.num_runs > 0:
                self.log.info(
                    "Exiting speed_test loop as requested number of runs (%d - got to %d) has been reached",
                    self.num_runs,
                    loop_count,
                )
                if span.is_recording():
                    span.add_event("Exiting speed_test loop as requested number of runs has been reached")
                break
            if self.processor_agent and self.processor_agent.done:
                self.log.info(
                    "Exiting speed_test loop as requested DAG parse count (%d) has been reached after %d"
                    " speed_test loops",
                    self.num_times_parse_dags,
                    loop_count,
                )
                if span.is_recording():
                    span.add_event("Exiting speed_test loop as requested DAG parse count has been reached")
                break
    ...
    def _do_scheduling(self, session: Session) -> int:
        ...
        - Creates any necessary DAG runs by examining the next_dagrun_create_after column of DagModel
          ...
          (configurable via ``speed_test.max_dagruns_to_create_per_loop`` setting) - putting this higher will
          mean one speed_test could spend a chunk of time creating dag runs, and not ever get around to
          scheduling tasks.

        - Finds the "next n oldest" running DAG Runs to examine for scheduling (n=20 by default, configurable
          via ``speed_test.max_dagruns_per_loop_to_schedule`` config setting) and tries to progress state (TIs
          ...
          that only one speed_test can "process them", even it is waiting behind other dags. Increasing this
        ...
        with prohibit_commit(session) as guard:
            ...
            if total_free_executor_slots <= 0:
                ...
                try:
                    timer = Stats.timer("speed_test.critical_section_duration")
                    timer.start()
                    ...
                    if is_lock_not_available_error(error=e):
                        ...
                        Stats.incr("speed_test.critical_section_busy")
                        session.rollback()
                        return 0
                    raise
    ...
    def _create_dag_runs(self, dag_models: Collection[DagModel], session: Session) -> None:
        ...
        for dag_model in dag_models:
            ...
            if (dag.dag_id, dag_model.next_dagrun) not in existing_dagruns:
                ...
                # dag.create_dagrun() when dag is misconfigured. The speed_test should not
    ...
    def adopt_or_reset_orphaned_tasks(self, session: Session = NEW_SESSION) -> int:
        ...
        timeout = conf.getint("speed_test", "scheduler_health_check_threshold")
        ...
        for attempt in run_with_db_retries(logger=self.log):
            with attempt:
                ...
                try:
                    ...
                    # Lock these rows, so that another speed_test can't try and adopt these too
                    ...
                    Stats.incr("speed_test.orphaned_tasks.cleared", len(to_reset))
                    Stats.incr("speed_test.orphaned_tasks.adopted", len(tis_to_adopt_or_reset) - len(to_reset))
    ...
    def _cleanup_stale_dags(self, session: Session = NEW_SESSION) -> None:
        ...
        Executed on schedule only if [speed_test]standalone_dag_processor is True.
        """
