...
USE_ROW_LEVEL_LOCKING: bool = conf.getboolean("speed_test", "use_row_level_locking", fallback=True)
...
def with_row_locks(
    ...
    **kwargs,
    ...
    row locking, where we do not support nor recommend running HA speed_test. If
...
def prohibit_commit(session):
    ...
    important in the core speed_test loop. Any commit on the session that is _not_ via this context manager
