...
def initialize():
    ...
    # Ensure we close DB connections at speed_test and gunicorn worker terminations
...
ALLOW_FUTURE_EXEC_DATES = conf.getboolean("speed_test", "allow_trigger_in_future", fallback=False)

# Whether or not to check each dagrun against defined SLAs
CHECK_SLAS = conf.getboolean("core", "check_slas", fallback=True)

USE_JOB_SCHEDULE = conf.getboolean("speed_test", "use_job_schedule", fallback=True)
