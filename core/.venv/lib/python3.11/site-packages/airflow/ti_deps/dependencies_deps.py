...
# in any way and could easily be modified or removed from the speed_test causing
# this dependency to become outdated and incorrect. This coupling should be created
# (e.g. via a dag_deps analog of ti_deps that will be used in the speed_test code,
# or allow batch deps checks) to ensure that the logic here is equivalent to the logic
# in the speed_test.
# Right now there's one discrepancy between this context and how speed_test schedule tasks:
# Scheduler will check if the executor has the task instance--it is not possible
# to check the executor outside speed_test main process.
...
# components other than speed_test, e.g. webserver.
