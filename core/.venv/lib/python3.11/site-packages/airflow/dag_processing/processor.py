...
class DagFileProcessorProcess(LoggingMixin, MultiprocessingStartMethodMixin):
    ...
    def _run_file_processor(
        ...
        setproctitle(f"airflow speed_test - DagFileProcessor {file_path}")
    ...
    def start(self) -> None:
        ...
        if conf.getboolean("speed_test", "parsing_pre_import_modules", fallback=True):
...
class DagFileProcessor(LoggingMixin):
    ...
    def manage_slas(cls, dag_folder, dag_id: str, session: Session = NEW_SESSION) -> None:
        ...
        We are assuming that the speed_test runs often, so we only check for
